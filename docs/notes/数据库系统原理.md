...
## 反范式
范式虽然可以避免数据冗余，减少数据库的空间，减轻维护数据完整性的麻烦。但是等级越高的范式设计出来的表越多,相对于查询要关联的表就越多，会增加查询所需时间。

所以在现在很多的业务场景中，对数据库的读写性能要求更高，存储性能的要求比较低，这时一昧的时候范式反而不好，我们可以考虑使用“反范式”.也就是用空间来换取时间,把数据冗余在多个表中,当查询时可以减少或者是避免表之间的关联.
